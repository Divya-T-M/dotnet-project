...
namespace DemoProject.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult Index(Employee emp, string action, int currentPage = 1, int pageSize = 10)
        {
            ...
            var employeeDataByState = employeeData.GroupBy(x => x.State);
            ViewBag.eployeeState = employeeDataByState;
            var ads = employeeDataByState.Select(x => x.Key == "Kerala");
            ...
            if (employeeData != null)
            {
                ...
                //employeeData = employeeData.DistinctBy(x => x.State).ToList();
                //ViewBag.EmployeeList = employeeData;


                //////////////// Grid
                var empDataByState = employeeData.GroupBy(x => x.State).Select(g => new
                {
                    State = g.Key,
                    Id = g.Select(x => x.Id).First(),
                    TotalVolVar = (float)g.Sum(x => x.VolVar),
                    TotalRevVar = (float)g.Sum(x => x.RevVar),
                    EmployeeData = g.ToList()  // Har group ke andar ke data ka collection
                }).ToList();
                ViewBag.EmployeeList = empDataByState;
                /////////////////////////////


                //var empDataByState = employeeData.GroupBy(x => x.State).Sum(x=>x.VolVar);
                ViewBag.DataCount = empDataByState.Count();
                //foreach (var item in empDataByState)
                //{
                //    ViewBag.RevVarTotal = item.Sum(x => x.RevVar);
                //    ViewBag.VolVarTotal = item.Sum(x => x.VolVar);
                //}

                //ViewBag.SumData = employeeData.GroupBy(x => x.State).Select(g =>
                //{
                //    double totalVol = g.Sum(item => item.VolVar);
                //    double totalRev = g.Sum(item => item.RevVar);
                ...
                //var employeeDataByState11 = employeeData.GroupBy(x => x.State).Select(group =>
                //{
                //    double totalVol = group.Sum(item => item.VolVar1 + item.VolVar2);
                //    double totalRev = group.Sum(item => item.RevVar1 + item.RevVar2);
                //    return new
                //    {
                //        State = group.Key,
                //        VolVarTotal = totalVol,
                //        RevVarTotal = totalRev
        ...
        [HttpGet]
        public IActionResult GetEmployeeState(DateTime StartDate, DateTime EndDate, string State)
        {

            var employeeData = _dataAccessLayer.GetEmployees(StartDate, EndDate);
            var employeeDataByState = employeeData.GroupBy(x => x.State);
            var ads = employeeDataByState.Where(x => x.Key == State);
        ...
        {
            // Split the string of dates into an array
            string[] dateStrings = dates.Split(',');

            // Parse each date string into DateTime objects
            List<DateTime> selectedDates = dateStrings.Select(dateString =>
            {
            ...
            // Pass the list of selected dates to your data access layer method
